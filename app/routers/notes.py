...
@router.post("/note/create", status_code=status.HTTP_201_CREATED, tags=["note"])
...
@router.get("/note/list", tags=["note"])
def note_list(db: Session = Depends(get_db)):
    all_notes = db.query(models.Note).all()
    return all_notes


@router.get("/note/{id}", status_code=200, tags=["note"])
...
@router.get("/note/by-user-id/{user_id}", status_code=200, tags=["note"])
...
@router.delete(
    "/note/delete/{id}", status_code=status.HTTP_204_NO_CONTENT, tags=["note"]
)
...
@router.put("/note/update/{id}", status_code=status.HTTP_202_ACCEPTED, tags=["note"])
